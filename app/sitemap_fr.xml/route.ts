...
function urlBlockFR(id: number, lastmod: string) {
  const fr = `${HOST}/fr/Projet/Detail/${id}`;
  const en = `${HOST}/en/Projet/Detail/${id}`; // garder la route réelle actuelle
  return `
  <url>
    <loc>${fr}</loc>
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  const urls = withFallback.map(r => urlBlockFR(r.id, isoDate(r.updated_at || r.updatedAt || r.created_at))).join('\n');
